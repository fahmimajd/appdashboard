    ...
    <form action="{{ route('vpn.store') }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <select id="desa_id" name="desa_id" 
                ...
                @foreach($desas as $d)
                    <option value="{{ $d->kode_desa }}" {{ old('desa_id') == $d->kode_desa ? 'selected' : '' }}>
                        {{ $d->nama_desa }}
                    </option>
                @endforeach
            </select>
            ...
            <div class="md:col-span-2">
                ...
                <div class="relative">
                    <input type="password" id="password" name="password" placeholder="Minimal 6 karakter"
                           class="w-full rounded-lg border-gray-300 focus:border-blue-500 focus:ring focus:ring-blue-200 transition duration-200 pr-10">
