...
@section('title', 'Pending Approval Belum Akte')
@section('subtitle', 'Daftar data belum akte yang menunggu approval')
...
<div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
    ...
    <div class="flex items-center justify-between mb-6">
        <div>
            <a href="{{ route('belum_akte.index') }}" class="text-sm text-gray-500 hover:text-blue-600 flex items-center gap-1 mb-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                </svg>
                Kembali ke Daftar Belum Akte
            </a>
    ...
    @if($pendingData->isEmpty())
        <div class="text-center py-12">
            ...
            <a href="{{ route('belum_akte.index') }}" class="inline-block mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-200">
                Kembali ke Daftar
            </a>
        </div>
    @else
            ...
            <div class="border border-yellow-200 rounded-xl bg-yellow-50 p-6">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between mb-4 pb-4 border-b border-yellow-200">
                    ...
                    <div class="flex gap-2 mt-4 md:mt-0">
                        <form action="{{ route('belum_akte.approve-all', $item->nik) }}" method="POST" class="inline-block" onsubmit="return confirm('Approve semua perubahan untuk data ini?')">
                            @csrf
                        ...
                        <form action="{{ route('belum_akte.reject-all', $item->nik) }}" method="POST" class="inline-block" onsubmit="return confirm('Tolak semua perubahan untuk data ini?')">
                            @csrf
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-yellow-200">
                            @php
                                $fieldLabels = \App\Models\BelumAkteApprovalLog::$fieldLabels;
                            @endphp
                                ...
                                <tr class="hover:bg-yellow-100 transition">
                                    ...
                                    <td class="py-3 px-3 text-right">
                                        <div class="flex items-center justify-end gap-2">
                                            <form action="{{ route('belum_akte.approve-field', $item->nik) }}" method="POST" class="inline-block">
                                                @csrf
                                            ...
                                            <form action="{{ route('belum_akte.reject-field', $item->nik) }}" method="POST" class="inline-block" onsubmit="return confirm('Tolak perubahan ini?')">
                                                @csrf
                                                <input type="hidden" name="field_name" value="{{ $fieldName }}">
