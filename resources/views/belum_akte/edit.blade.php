...
@section('title', 'Edit Data Belum Akte')
@section('subtitle', 'Ubah data belum memiliki Akte Kelahiran')

@section('content')
<div class="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
    <!-- Header -->
    <div class="mb-6">
        <a href="{{ route('belum_akte.index') }}" class="inline-flex items-center text-gray-600 hover:text-gray-900 transition duration-200">
            ...
            <ul class="text-sm ml-7 space-y-1">
                @foreach($data->getPendingFields() as $field)
                    @php
                        ...
                        $label = \App\Models\BelumAkteApprovalLog::$fieldLabels[$field] ?? $field;
                    @endphp
    ...
    <!-- Form -->
    <form action="{{ route('belum_akte.update', $data->nik) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="w-full px-4 py-3 bg-gray-100 border border-gray-300 rounded-lg text-gray-700">
                    {{ $data->jenis_klmin == 'L' ? 'Laki-laki' : 'Perempuan' }}
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label for="no_akta_kelahiran" class="block text-sm font-medium text-gray-700 mb-2">
                    No Akta Kelahiran
                </label>
                <input type="text" name="no_akta_kelahiran" id="no_akta_kelahiran" 
                    value="{{ old('no_akta_kelahiran', $data->no_akta_kelahiran) }}"
                    placeholder="Masukkan nomor akta kelahiran"
                    maxlength="50"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition duration-200 @error('no_akta_kelahiran') border-red-500 @enderror">
